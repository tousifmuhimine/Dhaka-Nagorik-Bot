...
def build_vector_store(
    *,
    ...
    selected_backend = (backend or os.getenv("VECTOR_STORE_BACKEND") or "supabase").strip().lower()
    if selected_backend != "supabase":
        raise ValueError(
            "Only Supabase pgvector is supported in this deployment. "
            "Set VECTOR_STORE_BACKEND=supabase."
        )
