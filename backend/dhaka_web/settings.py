...
DOCUMENT_OUTPUT_DIR = Path(os.getenv('DOCUMENT_OUTPUT_DIR', 'generated_docs'))
if not DOCUMENT_OUTPUT_DIR.is_absolute():
    DOCUMENT_OUTPUT_DIR = BASE_DIR / DOCUMENT_OUTPUT_DIR
POLICY_DIRECTORY = Path(os.getenv('POLICY_DIRECTORY', BASE_DIR / '_archive'))
ENABLE_SUPABASE_STORAGE = _env_bool('ENABLE_SUPABASE_STORAGE', default=True)
...
# In local development, allow common LAN frontend origins so Next.js can be used
# from another device on the same network without manual per-IP updates.
if DEBUG:
...
if not USE_SUPABASE_STORAGE:
    raise ImproperlyConfigured(
        'Supabase Storage is required. Set ENABLE_SUPABASE_STORAGE=true with valid '
